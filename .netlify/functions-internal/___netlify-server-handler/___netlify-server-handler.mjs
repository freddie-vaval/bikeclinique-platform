...
} from '/var/task/bikeclinique-platform/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/bikeclinique-platform/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/bikeclinique-platform')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/bikeclinique-platform',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/bikeclinique-platform/.netlify/dist/run/handlers/server.js')
